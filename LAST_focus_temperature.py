...
if  not os.path.isdir(outdir):
    os.mkdir(outdir)
time_span_stamp = pla.generate_time_span_str(N_days,N_show,startdate,enddate)
...
if  localrun:
    ...
    tracking = pla.read_DB1(client=client,
                        db_name ='operation_strings', 
                        ...
                        N_days = N_days,
                        N_read = N_show)
    tracking =  pla.basic_processing_tracking(tracking)
    # tracking = pla.filter_N_days(tracking, N_days, N_show)
    tracking = pla.separate_by_mount(tracking)
    tracking_groups = pla.tracking_windows(tracking) 
    #%% RA data
...
if plots['Alt_for_each_Focus']:
    pla.plot_alt_vs_hour(df = focus_groups, 
